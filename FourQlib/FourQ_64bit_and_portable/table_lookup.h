...
/*
 * Usage: mLSB-Comb table lookup implimentation
 * Author: Mengqing Yang
 * Time: 2023/03/03 15:48 +8
 * Mail: yangmengqing\@pku.edu.cn
 */
void table_lookup_fixed_base_new_var(point_precomp_t* table, point_precomp_t P, unsigned int digit, unsigned int sign)
{ // Var-time table lookup to extract a point represented as (x+y,y-x,2t) corresponding to extended twisted Edwards coordinates (X:Y:Z:T) with Z=1
  // Inputs: sign, digit, table containing VPOINTS_FIXEDBASE = 2^(W_FIXEDBASE-1) points
  // Output: if sign=0 then P = table[digit], else if (sign=-1) then P = -table[digit]
    ...
    point[0] = _mm256_loadu_si256((__m256i*)table[digit]->xy);                  // point = table[0] 
    point[1] = _mm256_loadu_si256((__m256i*)table[digit]->yx);  
    point[2] = _mm256_loadu_si256((__m256i*)table[digit]->t2); 
    ...
    point[0] = _mm256_loadu_pd((double const*)table[digit]->xy);                // point = table[0] 
    point[1] = _mm256_loadu_pd((double const*)table[digit]->yx);  
    point[2] = _mm256_loadu_pd((double const*)table[digit]->t2);  
    ...
    ecccopy_precomp_fixed_base(table[digit], point);                             // point = table[0]
 ...
 * Usage: mLSB-Comb table lookup implimentation
 * Author: Mengqing Yang
 * Time: 2023/03/03 16:35 +8
 * Mail: yangmengqing\@pku.edu.cn
 */
void table_lookup_fixed_base_new_const(point_precomp_t* table, point_precomp_t P, unsigned int digit, unsigned int sign)
{ // Constant-time table lookup to extract a point represented as (x+y,y-x,2t) corresponding to extended twisted Edwards coordinates (X:Y:Z:T) with Z=1
  // Inputs: sign, digit, table containing VPOINTS_FIXEDBASE = 2^(W_FIXEDBASE-1) points
  // Output: if sign=0 then P = table[digit], else if (sign=-1) then P = -table[digit]
