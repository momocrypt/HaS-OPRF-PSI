...
/*
 * Usage: Calculate $c = ab mod r$ with Montgomery Modual Multiplication.
 * Author: Mengqing Yang
 * Time: 2023/02/27 15:06 +8
 * Mail: yangmengqing\@pku.edu.cn
 */
void Scalar_mul(digit_t* a, digit_t* b, digit_t* c)
{
    digit_t ab[NWORDS_ORDER];
    Montgomery_multiply_mod_order(a, b, ab);
	Montgomery_multiply_mod_order(ab, (digit_t*)&Montgomery_Rprime, c);
    // modulo_order(c, c); // test?
}
